...
public sealed class BattleSpawnDirector
{
    ...
    private float _endlessTradeoffSpawnIntervalScale = 1f;
    ...
    public void Initialize(int stage, StageDefinition stageData, CombatTuning combat, IEnumerable<UnitDefinition> enemyRoster)
    {
        ...
        _endlessTradeoffSpawnIntervalScale = 1f;
        ...
        Action<string> setStatus)
    {
        ...
        _enemySpawnTimer = Mathf.Max(
            ...
            (_rng.RandfRange(_stageData.EnemySpawnMin, _stageData.EnemySpawnMax) * spawnIntervalScale * _endlessTradeoffSpawnIntervalScale) / pressure);
    }
        ...
        Action<string> setStatus)
    {
        ...
        while (elapsed + 0.001f >= _nextEndlessWaveTime)
        {
            ...
            {
                var bossCheckpoint = EndlessBossCheckpointCatalog.GetForWave(EndlessWaveNumber, _endlessRouteId);
    ...
    public void SetEndlessTradeoffSpawnIntervalScale(float value)
    {
        _endlessTradeoffSpawnIntervalScale = Mathf.Max(0.1f, value);
    }
    ...
    private void QueueEndlessWave(float executeAt, int waveNumber)
    {
        ...
        if (_endlessRouteId == RouteCatalog.ThornwallId && waveNumber >= 3 && waveNumber % 3 == 1)
        {
            QueuePendingSpawn(GetEnemyById(GameData.EnemyRunnerId), ref executeAt, spawnInterval * 0.84f);
        }

        if (_endlessRouteId == RouteCatalog.ThornwallId && waveNumber >= 5 && waveNumber % 4 == 2)
        {
            QueuePendingSpawn(GetEnemyById(GameData.EnemyHowlerId), ref executeAt, spawnInterval * 0.9f);
        }

        if (_endlessRouteId == RouteCatalog.ThornwallId && waveNumber >= 6 && waveNumber % 5 == 0)
        {
            QueuePendingSpawn(GetEnemyById(GameData.EnemySaboteurId), ref executeAt, spawnInterval * 0.82f);
        }

        if (_endlessRouteId == RouteCatalog.BasilicaId && waveNumber >= 4 && waveNumber % 3 == 0)
        {
            QueuePendingSpawn(GetEnemyById(GameData.EnemySplitterId), ref executeAt, spawnInterval * 0.94f);
        }

        if (_endlessRouteId == RouteCatalog.BasilicaId && waveNumber >= 5 && waveNumber % 4 == 1)
        {
            QueuePendingSpawn(GetEnemyById(GameData.EnemySpitterId), ref executeAt, spawnInterval * 0.92f);
        }

        if (_endlessRouteId == RouteCatalog.BasilicaId && waveNumber >= 6 && waveNumber % 5 == 2)
        {
            QueuePendingSpawn(GetEnemyById(GameData.EnemyJammerId), ref executeAt, spawnInterval * 0.9f);
        }
        ...
        if (_endlessRouteId == RouteCatalog.SteppeId && waveNumber >= 3 && waveNumber % 3 == 0)
        {
            QueuePendingSpawn(GetEnemyById(GameData.EnemyRunnerId), ref executeAt, spawnInterval * 0.84f);
        }

        if (_endlessRouteId == RouteCatalog.SteppeId && waveNumber >= 5 && waveNumber % 4 == 1)
        {
            QueuePendingSpawn(GetEnemyById(GameData.EnemyHowlerId), ref executeAt, spawnInterval * 0.9f);
        }

        if (_endlessRouteId == RouteCatalog.SteppeId && waveNumber >= 6 && waveNumber % 5 == 2)
        {
            QueuePendingSpawn(GetEnemyById(GameData.EnemySaboteurId), ref executeAt, spawnInterval * 0.82f);
        }

        if (_endlessRouteId == RouteCatalog.GloamwoodId && waveNumber >= 4 && waveNumber % 3 == 1)
        {
            QueuePendingSpawn(GetEnemyById(GameData.EnemyRunnerId), ref executeAt, spawnInterval * 0.86f);
        }

        if (_endlessRouteId == RouteCatalog.GloamwoodId && waveNumber >= 5 && waveNumber % 4 == 2)
        {
            QueuePendingSpawn(GetEnemyById(GameData.EnemyJammerId), ref executeAt, spawnInterval * 0.92f);
        }

        if (_endlessRouteId == RouteCatalog.GloamwoodId && waveNumber >= 6 && waveNumber % 5 == 0)
        {
            QueuePendingSpawn(GetEnemyById(GameData.EnemySaboteurId), ref executeAt, spawnInterval * 0.84f);
        }

        if (_endlessRouteId == RouteCatalog.CitadelId && waveNumber >= 4 && waveNumber % 3 == 2)
        {
            QueuePendingSpawn(GetEnemyById(GameData.EnemySpitterId), ref executeAt, spawnInterval * 0.92f);
        }

        if (_endlessRouteId == RouteCatalog.CitadelId && waveNumber >= 5 && waveNumber % 4 == 1)
        {
            QueuePendingSpawn(GetEnemyById(GameData.EnemyHowlerId), ref executeAt, spawnInterval * 0.94f);
        }

        if (_endlessRouteId == RouteCatalog.CitadelId && waveNumber >= 6 && waveNumber % 5 == 0)
        {
            QueuePendingSpawn(GetEnemyById(GameData.EnemySaboteurId), ref executeAt, spawnInterval * 0.84f);
        }
        ...
        if (waveNumber >= 5 && waveNumber % 4 == 0)
        {
            var supportId = _endlessRouteId switch
            {
                ...
                RouteCatalog.SteppeId => GameData.EnemyHowlerId,
            ...
            if (_endlessRouteId == RouteCatalog.ThornwallId && waveNumber >= 8)
            {
                QueuePendingSpawn(GetEnemyById(GameData.EnemyHowlerId), ref executeAt, spawnInterval * 0.94f);
            }
            ...
            if (_endlessRouteId == RouteCatalog.SteppeId && waveNumber >= 8)
            {
                QueuePendingSpawn(GetEnemyById(GameData.EnemyBruteId), ref executeAt, spawnInterval * 1.02f);
            }

            if (_endlessRouteId == RouteCatalog.GloamwoodId && waveNumber >= 8)
            {
                QueuePendingSpawn(GetEnemyById(GameData.EnemyHowlerId), ref executeAt, spawnInterval * 0.96f);
            }

            if (_endlessRouteId == RouteCatalog.CitadelId && waveNumber >= 8)
            {
                QueuePendingSpawn(GetEnemyById(GameData.EnemyJammerId), ref executeAt, spawnInterval * 0.98f);
            }
        }
    ...
    {
        var definition = EndlessBossCheckpointCatalog.GetForWave(waveNumber, _endlessRouteId);
        ...
        QueueEnemyCopies(definition.BossId, 1, ref executeAt, spawnInterval * 1.18f);
    ...
    private UnitDefinition PickEndlessEnemyDefinition(int waveNumber)
    {
        ...
        var runnerWeight = waveNumber >= 2
            ? (_endlessRouteId switch
            {
                ...
                RouteCatalog.SteppeId => 4.5f,
                RouteCatalog.GloamwoodId => 3.8f,
                RouteCatalog.CitadelId => 2.8f,
                _ => 3.4f
            }) + (waveNumber * 0.12f)
            : 0f;
        var bloaterWeight = waveNumber >= 4
            ? (_endlessRouteId switch
            {
                ...
                RouteCatalog.SteppeId => 0.8f,
                RouteCatalog.GloamwoodId => 0.9f,
                RouteCatalog.CitadelId => 1.4f,
                _ => 1.8f
            })
            : 0f;
        ...
        var saboteurWeight = waveNumber >= 5
            ? (_endlessRouteId switch
            {
                ...
                RouteCatalog.SteppeId => 2.6f,
                RouteCatalog.GloamwoodId => 2.2f,
                RouteCatalog.CitadelId => 2.0f,
                _ => 0.9f
            }) + (waveNumber * 0.03f)
            : 0f;
        var howlerWeight = waveNumber >= 4
            ? (_endlessRouteId switch
            {
                ...
                RouteCatalog.SteppeId => 2.8f,
                RouteCatalog.GloamwoodId => 2.4f,
                RouteCatalog.CitadelId => 2.1f,
                _ => 0.8f
            }) + (waveNumber * 0.03f)
            : 0f;
        var jammerWeight = waveNumber >= 6
            ? (_endlessRouteId switch
            {
                ...
                RouteCatalog.SteppeId => 0.4f,
                RouteCatalog.GloamwoodId => 2.6f,
                RouteCatalog.CitadelId => 2.0f,
                _ => 0.2f
            }) + (waveNumber * 0.02f)
            : 0f;
        ...
        var splitterWeight = waveNumber >= 6
            ? (_endlessRouteId switch
            {
                ...
                RouteCatalog.SteppeId => 1.0f,
                RouteCatalog.GloamwoodId => 0.9f,
                RouteCatalog.CitadelId => 2.2f,
                _ => 2.1f
            }) + (waveNumber * 0.04f)
            : 0f;
        var crusherWeight = waveNumber >= 5 ? 2.1f + (waveNumber * 0.05f) : 0f;
        var shieldwallWeight = waveNumber >= 10 ? 2.5f + (waveNumber * 0.08f) : 0f;
        var lichWeight = waveNumber >= 20
            ? (_endlessRouteId switch
            {
                RouteCatalog.QuarantineId => 2.7f,
                RouteCatalog.FoundryId => 0.8f,
            ...
            : 0f;
        var siegetowerWeight = waveNumber >= 25
            ? (_endlessRouteId switch
            {
                RouteCatalog.FoundryId => 1.2f,
                RouteCatalog.CitadelId => 1.0f,
                RouteCatalog.BasilicaId => 0.8f,
                _ => 0.5f
            }) + (waveNumber * 0.02f)
            : 0f;
        var mirrorWeight = waveNumber >= 15
            ? (_endlessRouteId switch
            {
                RouteCatalog.HarborId => 2.2f,
                RouteCatalog.FoundryId => 2.8f,
            ...
            : 0f;
        var tunnelerWeight = waveNumber >= 12
            ? (_endlessRouteId switch
            {
                RouteCatalog.FoundryId => 2.6f,
                RouteCatalog.QuarantineId => 2.4f,
        ...
        if (_endlessRouteId == RouteCatalog.FoundryId)
        {
            ...
            howlerWeight *= 1.2f;
            shieldwallWeight *= 1.22f;
            siegetowerWeight *= 1.3f;
            tunnelerWeight *= 1.18f;
        }
        ...
        if (_endlessRouteId == RouteCatalog.QuarantineId)
        {
            ...
            lichWeight *= 1.28f;
            mirrorWeight *= 1.14f;
            tunnelerWeight *= 1.12f;
        }

        if (_endlessRouteId == RouteCatalog.ThornwallId)
        {
            bloaterWeight *= 0.76f;
            bruteWeight *= 0.98f;
            runnerWeight *= 1.22f;
            saboteurWeight *= 1.36f;
            howlerWeight *= 1.28f;
            spitterWeight *= 0.86f;
            splitterWeight *= 0.7f;
            crusherWeight *= 1.08f;
            shieldwallWeight *= 1.16f;
            mirrorWeight *= 1.08f;
        }

        if (_endlessRouteId == RouteCatalog.BasilicaId)
        {
            runnerWeight *= 0.78f;
            bloaterWeight *= 0.84f;
            bruteWeight *= 1.06f;
            saboteurWeight *= 0.92f;
            howlerWeight *= 1.18f;
            jammerWeight *= 1.42f;
            spitterWeight *= 1.32f;
        ...
        if (_endlessRouteId == RouteCatalog.MireId)
        {
            ...
            crusherWeight *= 1.08f;
            tunnelerWeight *= 1.22f;
            mirrorWeight *= 0.88f;
        }

        if (_endlessRouteId == RouteCatalog.SteppeId)
        {
            bloaterWeight *= 0.72f;
            bruteWeight *= 1.08f;
            runnerWeight *= 1.34f;
            saboteurWeight *= 1.32f;
            howlerWeight *= 1.26f;
            spitterWeight *= 0.82f;
            splitterWeight *= 0.68f;
            crusherWeight *= 1.12f;
            shieldwallWeight *= 0.88f;
            tunnelerWeight *= 1.14f;
        }

        if (_endlessRouteId == RouteCatalog.GloamwoodId)
        {
            bloaterWeight *= 0.78f;
            bruteWeight *= 0.96f;
            runnerWeight *= 1.16f;
        ...
        if (_endlessRouteId == RouteCatalog.CitadelId)
        {
            runnerWeight *= 0.96f;
            bloaterWeight *= 0.9f;
            bruteWeight *= 1.12f;
            ...
            jammerWeight *= 1.18f;
            spitterWeight *= 1.16f;
            splitterWeight *= 1.08f;
            crusherWeight *= 1.18f;
            shieldwallWeight *= 1.14f;
            lichWeight *= 1.16f;
            siegetowerWeight *= 1.22f;
            mirrorWeight *= 1.1f;
            tunnelerWeight *= 1.1f;
        }
        ...
        var total = walkerWeight + runnerWeight + bloaterWeight + bruteWeight + saboteurWeight + howlerWeight + jammerWeight + spitterWeight + splitterWeight + crusherWeight + shieldwallWeight + lichWeight + siegetowerWeight + mirrorWeight + tunnelerWeight;
        if (total <= 0f)
        {
        ...
        roll -= crusherWeight;
        if (roll < shieldwallWeight)
        {
            return GetEnemyById(GameData.EnemyShieldWallId);
        }

        roll -= shieldwallWeight;
        if (roll < lichWeight)
        {
            return GetEnemyById(GameData.EnemyLichId);
        }

        roll -= lichWeight;
        if (roll < siegetowerWeight)
        {
            return GetEnemyById(GameData.EnemySiegeTowerId);
        }

        roll -= siegetowerWeight;
        if (roll < mirrorWeight)
        {
            return GetEnemyById(GameData.EnemyMirrorId);
        }

        return GetEnemyById(GameData.EnemyTunnelerId);
    }
    ...
    private static int GetEndlessEnemyCost(string unitId)
    {
        return unitId switch
        {
            ...
            GameData.EnemyShieldWallId => 2,
            GameData.EnemyLichId => 3,
            GameData.EnemySiegeTowerId => 4,
            GameData.EnemyMirrorId => 2,
            GameData.EnemyTunnelerId => 2,
