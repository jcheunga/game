...
TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/http-room-reset-smoke.XXXXXX")"
SERVER_LOG="$TMP_DIR/server.log"
REQUEST_LOG="$TMP_DIR/request.json"
SMOKE_LOG="$TMP_DIR/smoke.log"
RUNNER_CSPROJ="$TMP_DIR/RoomResetSmoke.csproj"
RUNNER_PROGRAM="$TMP_DIR/Program.cs"
SERVER_PID=""
SMOKE_STATUS=0
PORT="${HTTP_ROOM_RESET_SMOKE_PORT:-$((20400 + RANDOM % 1000))}"
...
cleanup() {
	...
	if [[ "${KEEP_HTTP_ROOM_RESET_SMOKE_LOGS:-0}" != "1" && $SMOKE_STATUS -eq 0 ]]; then
		rm -rf "$TMP_DIR"
	fi
}
trap cleanup EXIT
...
class Handler(BaseHTTPRequestHandler):
    def do_POST(self):
        ...
        response = {
            "status": "accepted",
            "message": "stub room reset accepted",
            "roomId": "ROOM-HOST-11",
            "boardCode": "CH-05-BLK-5110",
            "ticketId": "HOST-HTTP-01",
            "actionId": "reset_round",
            "readyState": False,
            "processedAtUnixSeconds": 1710003000
        }
...
internal static class Program
{
    private static int Main(string[] args)
    {
        ...
        var request = new OnlineRoomActionRequest
        {
            ...
            ActionId = "reset_round",
            ReadyState = false,
            RequestedAtUnixSeconds = 1710002950
        };

        var result = provider.SendAction(ticket, request);
        if (!string.Equals(result.Status, "accepted", StringComparison.OrdinalIgnoreCase))
        {
            Console.Error.WriteLine($"expected accepted room reset, got {result.Status}");
            return 1;
        }

        if (!string.Equals(result.ActionId, "reset_round", StringComparison.OrdinalIgnoreCase))
        {
        ...
        Console.WriteLine(
            $"ROOM_RESET_SMOKE PASS  |  provider {result.ProviderDisplayName}  |  room {result.RoomId}  |  action {result.ActionId}");
        return 0;
    }
}
CS

echo "Running HTTP online room reset smoke..."
...
if [[ $SMOKE_STATUS -ne 0 ]]; then
	echo "HTTP online room reset smoke failed."
	echo "--- smoke log ---"
	cat "$SMOKE_LOG"
	echo "--- server log ---"
	cat "$SERVER_LOG"
...
if ! grep -q "ROOM_RESET_SMOKE PASS" "$SMOKE_LOG"; then
	echo "Room reset smoke exited cleanly but did not report pass."
	cat "$SMOKE_LOG"
	exit 1
fi

if [[ ! -s "$REQUEST_LOG" ]]; then
	echo "Room reset stub did not capture a request."
	cat "$SMOKE_LOG"
	exit 1
fi

if ! grep -q '"actionId":"reset_round"' "$REQUEST_LOG"; then
	echo "Room reset request payload did not include the expected reset action."
	cat "$REQUEST_LOG"
	exit 1
fi

echo "HTTP online room reset smoke passed."
echo "Smoke summary: $(grep 'ROOM_RESET_SMOKE PASS' "$SMOKE_LOG" | tail -n 1)"
